package com.yzss.custom.View.Generally;
...
public class FancyButton extends LinearLayout {
	...
	/**
	 * Initialize Button dependencies - Initialize Button Container : The
	 * LinearLayout - Initialize Button TextView - Initialize Button Icon -
	...
	private void initAttributsArray(TypedArray attrsArray) {
		...
		mDefaultBackgroundColor = attrsArray.getColor(
				R.styleable.FancyButtonsAttrs_fb_defaultColor,
				mDefaultBackgroundColor);
		mFocusBackgroundColor = attrsArray.getColor(
				R.styleable.FancyButtonsAttrs_fb_focusColor,
				mFocusBackgroundColor);

		mDefaultTextColor = attrsArray.getColor(
				R.styleable.FancyButtonsAttrs_fb_textColor, mDefaultTextColor);
		// if default color is set then the icon's color is the same (the
		// default for icon's color)
		...
		mBorderColor = attrsArray.getColor(
				R.styleable.FancyButtonsAttrs_fb_borderColor, mBorderColor);
		mBorderWidth = (int) attrsArray.getDimension(
				R.styleable.FancyButtonsAttrs_fb_borderWidth, mBorderWidth);

		mRadius = (int) attrsArray.getDimension(
				R.styleable.FancyButtonsAttrs_fb_radius, mRadius);
		mFontIconSize = (int) attrsArray.getDimension(
		...
		mIconPaddingLeft = (int) attrsArray.getDimension(
				...
				mIconPaddingLeft);
		mIconPaddingRight = (int) attrsArray.getDimension(
				R.styleable.FancyButtonsAttrs_fb_iconPaddingRight,
				mIconPaddingRight);
		mIconPaddingTop = (int) attrsArray.getDimension(
				R.styleable.FancyButtonsAttrs_fb_iconPaddingTop,
				mIconPaddingTop);
		mIconPaddingBottom = (int) attrsArray.getDimension(
				R.styleable.FancyButtonsAttrs_fb_iconPaddingBottom,
				mIconPaddingBottom);

		mGhost = attrsArray.getBoolean(R.styleable.FancyButtonsAttrs_fb_ghost,
				mGhost);

		String text = attrsArray
				.getString(R.styleable.FancyButtonsAttrs_fb_text);
		mIconPosition = attrsArray.getInt(
				R.styleable.FancyButtonsAttrs_fb_iconPosition, mIconPosition);

		String fontIcon = attrsArray
				.getString(R.styleable.FancyButtonsAttrs_fb_fontIconResource);

		String iconFontFamily = attrsArray
				.getString(R.styleable.FancyButtonsAttrs_fb_iconFont);
		String textFontFamily = attrsArray
				.getString(R.styleable.FancyButtonsAttrs_fb_textFont);

		Drawable icon = null;
		try {
			mIconResource = attrsArray
					.getDrawable(R.styleable.FancyButtonsAttrs_fb_iconResource);
	...
	private void setupBackground() {
		...
		if (mGhost) {
			drawable.setColor(getResources().getColor(
					android.R.color.transparent)); // Hollow Background
		} else {
		...
		if (mGhost) {
			drawable2.setColor(getResources().getColor(
					android.R.color.transparent)); // No focus color
		} else {
	...
	private void initializeButtonContainer() {
		...
		if (mIconResource == null && mFontIcon == null && getPaddingLeft() == 0
	 ...
	 *            : Padding Bottom
	 */
	public void setIconPadding(int paddingLeft, int paddingTop,
		...
		if (mIconView != null) {
			mIconView.setPadding(this.mIconPaddingLeft, this.mIconPaddingTop,
					this.mIconPaddingRight, this.mIconPaddingBottom);
		}
		if (mFontIconView != null) {
			mFontIconView.setPadding(this.mIconPaddingLeft,
					this.mIconPaddingTop, this.mIconPaddingRight,
	 ...
	 * FancyButton.POSITION_RIGHT, FancyButton.POSITION_TOP,
